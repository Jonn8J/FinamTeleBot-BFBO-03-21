...
def get_df_tf0(security_board, ticker, timeframe_0, period_sma_fast, period_sma_slow):
    """Считываем данные для обучения нейросети - вход - timeframe_0"""
    _filename = os.path.join(os.path.join(cur_run_folder, "data"), f"{security_board}.{ticker}_{timeframe_0}.txt")
    df = pd.read_csv(_filename, delimiter='\t')  # , index_col='datetime')
    if timeframe_0 in ["M1", "M15", "H1"]:
...
def get_df_t1(security_board, ticker, timeframe_1):
    """Считываем данные для обучения нейросети - выход - timeframe_1"""
    _filename = os.path.join(os.path.join(cur_run_folder, "data"), f"{security_board}.{ticker}_{timeframe_1}.txt")
    df = pd.read_csv(_filename, delimiter='\t')
    if timeframe_1 in ["M1", "M15", "H1"]:
        df['datetime'] = pd.to_datetime(df['datetime'], format='%d.%m.%Y %H:%M')
    else:
        df['datetime'] = pd.to_datetime(df['datetime'], format='%d.%m.%Y')
    return df.copy()
